...
class UnconfirmedPaymentsLoader:
    ...
    def _load(self):
        ...
        if os.path.isfile(self.transactions_store):
            logging.info('true')
            unconfirmed_transactions_list = transaction_list_decode(open(self.transactions_store).read())
            ...
            for unconfirmed_transaction in unconfirmed_transactions_list:
                ...
                #logging.debug('Loaded unconfirmed transactions has signature : {}'.format(iden))
        else:
            logging.info('false')
